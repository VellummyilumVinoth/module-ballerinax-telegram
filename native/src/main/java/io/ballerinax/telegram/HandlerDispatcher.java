...
package io.ballerinax.telegram;
...
/**
 * Dispatches an inbound webhook update to a {@code TelegramService} handler by name, if (and only
 * if) the attached service implementation declares that handler.
 *
 * <p>{@code TelegramService} declares no remote methods of its own (all nine handlers are
 ...
 * the pattern used by {@code ballerinax/whatsapp.business}'s {@code WhatsAppService} dispatch.</p>
 *
 * @since 0.1.0
 */
public final class HandlerDispatcher {
     ...
     * @param serviceObject the {@code TelegramService} implementation attached to the listener
     * @param methodName    the handler's name, e.g. {@code onMessage}
