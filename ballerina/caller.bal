...
# Acknowledges a Telegram webhook update. Declare it as a handler's optional second parameter
# (e.g. `onMessage(Message message, Caller caller)`) to take control of when the update is
# acknowledged — otherwise irrelevant unless the service is annotated `ServiceConfig` with
# `autoAck: false`; an unannotated service defaults to `autoAck: true` (automatic
# acknowledgement).
#
# ```ballerina
# remote function onMessage(telegram:Message message, telegram:Caller caller) returns error? {
#     check performSlowWork(message);
...
# `500 Internal Server Error` — a non-`2xx`, so Telegram's own webhook retry behavior (retry, then
...
#
# Telegram may redeliver an update if the acknowledgement is slow, dropped, or never sent — under
# either `autoAck` setting, not just `false`. Make handler processing idempotent, or deduplicate
# using `update_id`, rather than assuming a webhook update is delivered exactly once.
public isolated client class Caller {
    ...
    # Acknowledges the update by responding `200 OK` to Telegram. A no-op if the update was
    # already *successfully* acknowledged (automatically, or by an earlier call to this method). A
    # failed call does not mark the update as acknowledged, so a later call is not a no-op — it
    # attempts to acknowledge again.
    #
