...
/**
 * Analysis task that validates a service declaration attached to a Telegram {@code
 * telegram:Listener}.
 */
public class TelegramServiceAnalysisTask implements AnalysisTask<SyntaxNodeAnalysisContext> {

    private final TelegramServiceValidator serviceValidator = new TelegramServiceValidator();
    ...
    public void perform(SyntaxNodeAnalysisContext context) {
        ...
        if (!isTelegramService(context)) {
            return;
        }
        serviceValidator.validate(context);
    }

    private boolean isTelegramService(SyntaxNodeAnalysisContext context) {
        ...
        for (TypeSymbol listener : listeners) {
            if (isTelegramListener(listener)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isTelegramListener(TypeSymbol listener) {
        if (listener.typeKind() == TypeDescKind.UNION) {
            for (TypeSymbol member : ((UnionTypeSymbol) listener).memberTypeDescriptors()) {
                ...
                if (module.isPresent() && PluginUtils.isTelegramModule(module.get())) {
                    return true;
                }
            }
            return false;
        ...
        return module.isPresent() && PluginUtils.isTelegramModule(module.get());
    }
}
