...
import io.ballerinax.telegram.plugin.PluginConstants.CompilationErrors;

import java.util.List;
import java.util.Optional;

/**
 * Validates the remote functions declared on a {@code TelegramService} implementation: each must
 * be one of the nine documented handlers, with the expected parameter type and an {@code error?}
 * return type.
 */
public class TelegramServiceValidator {
    ...
    private boolean isExpectedEventType(TypeSymbol typeSymbol, String expectedSimpleName) {
        ...
        return name.isPresent() && name.get().equals(expectedSimpleName) &&
                module.isPresent() && PluginUtils.isTelegramModule(module.get());
    }
}
